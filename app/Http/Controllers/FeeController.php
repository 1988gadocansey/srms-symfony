...
//                    }
               ...
               if(empty($sql)){
                ...
                if ($feeLedger->save()) {
                    ...
                    if($level!="" &&$year!=""){
                        if($request->input('stno')!=""){
                    StudentModel::where('INDEXNO', $indexno)->update(array('BILL_OWING' => $billOwing, 'TELEPHONENO' => $phone,'SYSUPDATE'=>'1',"ALLOW_REGISTER"=>$status));
                        }
                        else{
                            StudentModel::where('INDEXNO', $indexno)->update(array('BILL_OWING' => $billOwing, 'TELEPHONENO' => $phone, 'LEVEL' => $level, 'YEAR' => $newyear,'SYSUPDATE'=>'1',"ALLOW_REGISTER"=>$status));
